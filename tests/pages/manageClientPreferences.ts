...
async manageClientPreferences() {
    await this.page.getByRole('link', { name: 'Manage Client Preferences' }).click();
    await this.page.getByLabel('Client:').selectOption('0');
    await expect(this.page.getByLabel('Client:')).toHaveValue('0');
    await expect(this.page.getByLabel('Client:')).toBeVisible();
    await expect(this.page.locator('#clientSelectWrapper').getByRole('button', { name: 'View' })).toBeVisible();
    await expect(this.page.getByRole('link', { name: 'Member Search' })).toBeVisible();
    await this.page.getByRole('link', { name: 'Scheduled Task Queue' }).click();
    await this.page.getByRole('link', { name: 'Eligible Member Queue' }).click();
    ...
    await this.page.getByRole('link', { name: 'Case And Claim Search' }).click();
    ...
    await this.page.getByLabel('Client:').selectOption('34');
    await this.page.getByRole('link', { name: 'Member Search' }).click();
    await this.page.getByRole('link', { name: 'Scheduled Task Queue' }).click();
    await this.page.getByRole('link', { name: 'Eligible Member Queue' }).click();
    await this.page.getByRole('link', { name: 'UM Request' }).click();
    await this.page.getByRole('link', { name: 'Correspondence' }).click();
