...
test.describe('STAGE - Manage System Messages Testing Suite', () => {
    test('Complete Manage System Messages test flow', async ({ page }) => {
        test.setTimeout(500000);

        const login = new loginPage(page);
        const nav = new navigationPage(page);
        const manageSystemMessage = new manageSystemMessages(page);
        ...
        try {
            ...
            await test.step('Test Manage Review Types', async () => {
                ...
                await manageSystemMessage.manageSystemMessages();
            });
        } catch (err) {
