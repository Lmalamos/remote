...
test.describe('STAGE - Call Processing Testing Suite', () => {
    test('Complete Call Processing test flow', async ({ page }) => {
        ...
        try {
            ...
            await test.step('Search for member and test Call Processing', async () => {
                ...
                await callProcessing.verifyCallProcessingData();
                await callProcessing.verifyCallProcessingList();
            });
        } catch (err) {
